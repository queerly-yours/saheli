...
export const categories: category[] = [
    ...
    },
    {
        ...
        decades: [],
        description: {
            en: [`From very early on, Saheli started periodically bringing out newsletters to share our work and perspectives on a range of contemporary issues. We also conducted surveys and in-depth studies on specific subjects and published special reports. Posters, leaflets, stickers and statements, etc were other kinds of campaign materials we produced. Then, there have been analytical and reflective souvenirs issued from time to time. We also designed and produced calendars as a means of raising funds to support our work. All along, we tried to widen our reach by creating content in Hindi and English. It may be of interest to note that in all these decades, none of our publications carry author names since they all emerge from collective experience and knowledge, including of those in the group who may not engage with the actual process of writing.`
            ],
...
if (archiveCategory) {
    const excludedFromArchives = ['13', '9', '14']
    archiveCategory.archiveList = categories.filter(category => !excludedFromArchives.includes(category.id)).map(category => ({
        ...
        subtitle: {
            en: category.subtitle?.en,
            hi: category.subtitle?.hi
        },
        archiveImg: category.archiveImg,
        images: categoryArchiveImages[category.id] ?? []
    }));

    archiveCategory.archiveList.push({
        id: 'misc',
        title: 'Miscellaneous',
        hindiTitle: 'अन्य',
        subtitle: {
        ...
        archiveImg: '',
        images: categoryArchiveImages['misc'] ?? []
    })
}
